...
<!-- filepath: resources/views/laptops/edit.blade.php -->
@extends('layouts.app')
...
<div class="min-h-screen bg-gray-50 py-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <nav class="flex mb-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li>
                        <div class="flex items-center">
                            <i class="fas fa-chevron-right text-gray-400 w-4 h-4"></i>
                            <a href="{{ route('laptops.show', $laptop) }}" class="ml-1 text-gray-700 hover:text-blue-600 md:ml-2">{{ $laptop->brand }} {{ $laptop->model }}</a>
                        </div>
                    </li>
                    <li aria-current="page">
                        <div class="flex items-center">
                            <i class="fas fa-chevron-right text-gray-400 w-4 h-4"></i>
                            <span class="ml-1 text-gray-500 md:ml-2">Редактиране</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <p class="mt-2 text-gray-600">Редактирай информацията за {{ $laptop->brand }} {{ $laptop->model }}</p>
        </div>

        <!-- Form -->
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <form action="{{ route('laptops.update', $laptop) }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    <label for="price" class="block text-sm font-medium text-gray-700 mb-2">
                        <i class="fas fa-dollar-sign mr-1"></i>Цена (лв.) *
                    </label>
                    <div class="relative">
                        <input type="number" 
                               name="price" 
                ...
                <div>
                    ...
                    <textarea name="description" 
                              ...
                              placeholder="Опиши особеностите и предимствата на този лаптоп...">{{ old('description', $laptop->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                    ...
                    <button type="submit" 
                            class="inline-flex items-center px-8 py-3 border border-transparent rounded-lg text-sm font-medium text-white bg-yellow-600 hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 transition-colors shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
